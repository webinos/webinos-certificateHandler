...
var Certificate = function(webinosMetaData) {
    ...
    function initializeCertificate() {
        CertContext.webinosMetaData= webinosMetaData;  
    ...
    this.validateConnection = function(conn, caCert, crlCert) {
        var tmp, tmp1, parseCACert = {};
        var parseCert = CertContext.parseCert(conn, "pem");
        var subject = parseCert.subject && parseCert.subject.CN.split(":") ;
        for (var i = 0 ; i < caCert.length; i = i + 1) {
            tmp = CertContext.parseCert(caCert[i], "pem");
            parseCACert[tmp.subject.CN] = tmp;
        }
        if (subject && (subject[0] === "Pzp")) { // Connection certificates
            // PZP Conn Certificate are signed by PZP CA which in turned is signed by PZH CA
            // PZH Conn Certificate are signed by PZH CA

            // There should be 2 certs in this list, we are going to follow order
            // First PZH CA, then PZP CA
            tmp = tmp1 = undefined;
            if (parseCert.issuer.CN && parseCACert[parseCert.issuer.CN]){
                tmp = (parseCACert[parseCert.issuer.CN]);
                tmp1 = parseCACert.hasOwnProperty(tmp.issuer.CN);
            }
            return (certificateManager.verifyCertificate(conn, caCert) && tmp1);
        } else {
            // PzhCA is self signed entity check if it is self signed
            // PZP CA in Virgin mode is self signed but in this state, no body can connect to it...
            // In enrolled state it should have certificate signed by PzhCA
            return (certificateManager.verifyCertificate(conn, caCert) && parseCACert.hasOwnProperty(parseCert.issuer.CN));
        }
    };
