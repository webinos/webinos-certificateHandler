...
// DCRYPT functions for xmldigsig
Certificate.dcrypt = function(){
	var certificateManager = require ("certificate_manager");
	
	return {
		createVerify: function (algo) {
			return (new certificateManager.Verify()).init(algo);
		},
		parseCert: function(cert, format){
			var convType = (format === "base64") ? 2 : 1;
			var parseCert =  certificateManager.parseCert(cert, convType);
			// compatibility with dcrypt (adds one last :)
			parseCert.public_key = parseCert.publicKey + ":";
			delete parseCert.publicKey;
			return parseCert;
		},
		parseCrl:  function(crl, format){
			var convType = (format === "base64") ? 2 : 1;
			return certificateManager.parseCrl(crl, convType);
		}
	}
};
