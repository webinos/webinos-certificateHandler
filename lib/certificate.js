...
var Certificate = function(webinosMetaData) {
    ...
    this.parseCert = function(cert, format){
        ...
        if (format === "base64") {
            certificate = new Buffer(cert, "base64").toString("utf8");
        }
        var parseCert =  certificateManager.parseCert(certificate);
    ...
    this.parseCrl = function(crl, format){
        var crl_ = crl;
        if (format === "base64") {
            crl_ = new Buffer(crl, "base64").toString("utf8");
        }
        var parseCrl =  certificateManager.parseCrl(crl_);
